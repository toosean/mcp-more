 ...
 * 获取错误的用户友好描述
 */
export const getOAuthErrorDescription = (error: any): string => {
  if (!error) {
  ...
  if (typeof error === 'object' && error.error) {
    const errorMap: Record<string, string> = {
      ...
      'unauthorized_client': 'The client is not authorized to request tokens using this method',
      ...
      'invalid_grant': 'The provided authorization code is invalid, expired, or revoked',
      'unsupported_grant_type': 'The grant type is not supported by the authorization server'
    };

    const description = errorMap[error.error] || error.error_description || error.error;
    return description;
  }
